...
— Запрещены любые другие заголовки.
— НЕ используй слова-заголовки: "КОРОТКО", "ВОПРОС", "ОТВЕТ", "ПОЛЬЗОВАТЕЛЬ" (ни в title, ни в text).
...
Пиши тепло, спокойно и нейтрально, без оценок и осуждения. Не используй обращения в третьем лице вроде "пользователь находится" — просто спокойно описывай, что есть.
Никаких вопросов. Никаких техник дыхания, медитации, телесных практик. Не предлагай «почитать что‑то» или «посмотреть фильм» как универсальный совет.
Только мягкое отражение фактов и один небольшой безопасный шаг без давления и без формулировок «ты должен» или «тебе надо».
Только три блока, только с этими заголовками по порядку: СЕЙЧАС, МОЖНО, НЕ ДЕЛАЙ.
`;
  } else if (mode === "guide") {
    description = `
Режим: Старший брат (guide).
Обращайся на «ты» напрямую, без третьего лица («пользователь», «он/она»). Тон: уважительный, без стыда и давления, но по делу.
Максимум один короткий вопрос (до 120 символов) и только если без него нельзя выбрать направление. Никаких опросников и списков вопросов подряд, никаких перечислений через «или / или / или».
Если вопрос не нужен — дай один конкретный шаг, который можно сделать за ≤30 минут.
...
Обращайся на «ты», говори прямо и конкретно, но без унижения и стыда. Фокус — на решении и действии, без длинных объяснений.
Если человек уходит в абстракции вроде «не знаю», «всё», «ничего» — можно задать один прямой вопрос‑фиксацию, но только один. Без заданий «составь список из 10…» и других громоздких инструкций.
Иначе дай один конкретный шаг, который можно начать делать за ≤30 минут.
...
`;
  ...
  if (
    ...
    restriction =
      "Пользователь уже дважды ответил уклончиво или 'не знаю' — сузь выбор до двух вариантов и помоги выбрать один. Без дополнительных вопросов или длинных списков.";
  } else if (previousKind === "question") {
    restriction =
      "В прошлом сообщении уже был вопрос, в этом просто поддержи или побуди к действию (без нового вопроса).";
  }
...
function fallbackAnswer(
  ...
  return NextResponse.json(
    {
      kind: "answer",
      blocks: [{ title: "", text: "Попробуй переформулировать одной фразой." }],
    },
    { status }
  );
}
...
export async function POST(request: Request) {
  try {
    ...
    const consecutiveUncertain =
      typeof body?.consecutiveUncertain === "number"
        ? body.consecutiveUncertain
        : undefined;
    const userPayload = `Ввод пользователя:\n${content}\n\nОтветь строго в json.`;
    ...
    const forbidden = ["коротко", "вопрос", "ответ", "пользователь"];
